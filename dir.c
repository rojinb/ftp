...
/* 
   Arguments: 
      directory - a pointer to a null terminated string that names a 
                  directory
      file - the file name we are looking for

   Returns
      -1 the named directory does not exist or you don't have permission
         to read it. or the file does not exist in this directory 
         or the file is not a regular file
      - int > 0 : size of the file named 

 
   This function takes the name of a directory and name of a file and 
   checks if the file named is a regular file inside directory
...
int getFile(char * directory, char* file) {
  ...
  for (dirEntry = readdir(dir);
    ...
    if (strcmp(file, dirEntry->d_name) != 0) {
      continue;
    } 
    if (dirEntry->d_type == DT_REG) {  // Regular file
      struct stat buf;
      ...
      stat(dirEntry->d_name, &buf);
      // change -1 to size when file found and is regular
      ret = buf.st_size;
